...
def get_source_db_credentials():
    """Get source database credentials from Secrets Manager"""
...
def get_primary_keys(table_name: str, token: str) -> List[str]:
    ...
    try:
        ...
        if not primary_keys:
            error_msg = f"No primary key found for table {SCHEMA_NAME}.{table_name}"
...
def get_foreign_keys(table_name: str, token: str) -> Dict[str, str]:
    """Get foreign key columns and their referenced tables from SOURCE database"""
    cache_key = f"{SOURCE_DB_SCHEMA}.{table_name}"
